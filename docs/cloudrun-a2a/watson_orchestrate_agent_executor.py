...
class WatsonOrchestrateAgentExecutor(AgentExecutor):
    """
    An AgentExecutor that bridges the A2A server with our WatsonOrchestrateAgent.
    """

    def __init__(self):
        self.agent = WatsonOrchestrateAgent()
    ...
    async def execute(
        ...
        try:
            ...
            logger.info(f"WatsonOrchestrateAgent for session {context.context_id} returned: '{result_text}'")

            # Create an artifact with the text part
            artifact = new_artifact(
                parts=[Part(root=TextPart(text=result_text))],
                name=f'watsonx_response_{context.task_id}',
            )
            ...
            logger.error(f"Error invoking WatsonOrchestrateAgent for session {context.context_id}: {e}", exc_info=True)
            # You could enqueue a failed task event here if desired
            raise ServerError(
                error=ValueError(f"Error invoking WatsonOrchestrateAgent: {e}")
            ) from e
